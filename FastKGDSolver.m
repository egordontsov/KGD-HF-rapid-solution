function [wvst,wvsx,lvst,etavst] = FastKGDSolver(t,xi,Cp,Ep,Kp,mup,Q0);
  %t - array of times
  ...
  %Q0 - injection rate per unit height
  ...
  [Omega,gamma,efficiency,delta,lambda] = KGDSolverScaled(tau,Km);
...
function [Omega,gamma,efficiency,delta,lambda] = KGDSolverScaled(tau,Km)
   
  %do not allow zero toughness 
  Km(Km<1e-30) = 1e-30;
  ...
  for ialphaiter = 1:3
    ...
    while (inneriter<ittmax) && (Residual>tol)

        if (inneriter==ittmax-1) && (ialphaiter==3)
           disp('No convergence'); 
           disp(Residual);
        end
        ...
        while (ittC<ittmax) && (ResC>tol)
           ittC = ittC+1;
           Chtest = Ch;
           Ch = th.^(1/6).*Kh.^(2/3)./alpha.^(1/2)./Qh.^(1/3).*(Bfunc(Kh,Ch,alpha)).^(1/3);          
           ResC = max(abs(Ch-Chtest));
        end

        Residual = max(((Kh-Kh0).^2+(Ch-Ch0).^2).^(1/2));
  ...
  %efficiency
  efficiency = 1-Ch.*alpha.^(3/2).*beta(alpha,3/2)./Bfunc(Kh,Ch,alpha);
...
function B = Bfunc(Kh,Ch,alpha)
  ...
  B = 2.^(1+delta).*(B0(1/2,lambda+1,1+delta))+Ch.*alpha.^(3/2).*beta(alpha,3/2);
...
function lambda = GetLambda(Kh,Ch,alpha)
  ...
  B0 = @(x,p1,p2) beta(p1,p2).*(1-betainc(x,p1,p2));
  fcn_B2 = 2.^(1+delta).*(B0(1/2,lam0+1,1+delta))+Ch.*alpha.^(3/2).*beta(alpha,3/2);
